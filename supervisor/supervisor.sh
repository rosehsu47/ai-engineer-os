...
#
# 每輪開一個全新的 `claude -p "/ai-work"` session（狀態都在 .ai/ 檔案裡，
...
# ---------- 狀態檔結構 lint（只偵測，不修復——修復永遠歸 /ai-work 的協定自癒） ----------
TAB_CHAR=$(printf '\t')
...
run_probe() { # headless 寫入實測：不走 /ai-work（不吃任務、不動 agent 狀態）
...
run_doctor() {
  ...
  # 目標 repo 的 skills
  miss=""
  for k in ai-work ai-review ai-task ai-answer ai-wrap; do
    [ -f "$REPO/.claude/skills/$k/SKILL.md" ] || miss="$miss $k"
  done
  if [ -z "$miss" ]; then d_ok "目標 repo skills 齊全（ai-work/ai-review/ai-task/ai-answer/ai-wrap）"
  ...
  else d_bad "checkpoint.json：$r" "下一輪 /ai-work 會自癒；急的話依 AI-RUNTIME schema 手動重置"; fi
  for k in backlog doing done; do
    r=$(lint_tasks "$REPO/.ai/tasks/$k.yaml" "$k")
    if [ "$r" = ok ]; then d_ok "${k}.yaml 結構"
    else d_bad "${k}.yaml：$r" "下一輪 /ai-work 會自癒（done.yaml 救不回是改名保留，不會清空）"; fi
  done
  # 旗標
...
# ---------- 主迴圈 ----------
# 啟動前狀態檔結構檢查（只警告不阻擋——壞檔自癒本身就是 /ai-work 的恢復路徑）
lint_bad=$(lint_state)
[ "$lint_bad" = 0 ] || log "state lint：${lint_bad} 個狀態檔結構異常（見上），預期下一輪 /ai-work 依協定自癒"
...
while [ "$iter" -lt "$MAX_ITER" ]; do
  ...
  if [ -f "$REPO/.ai/PAUSED" ]; then
    log "PAUSED 已有人類回覆，跑一輪 /ai-work 讓它消化並清旗"
  fi
  quota_check || exit 0
  ...
  ( cd "$REPO"; exec claude -p "/ai-work" --output-format json --model "$MODEL" $PERM_FLAG $SCHED_FLAGS $EXTRA_FLAGS ) >"$out" 2>"$err" &
  cpid=$!
  ...
  case "$class" in
    ...
    productive)
      ...
      if [ "$mtime_after" = "$mtime_before" ]; then
        ...
        if [ "$want_review" = 1 ]; then
          ...
          ( cd "$REPO"; exec claude -p "/ai-review" --output-format json --model "$MODEL" $PERM_FLAG $SCHED_FLAGS $EXTRA_FLAGS ) >"$rout" 2>>"$err"
      ...
      # 誤判不可以計失敗殺迴圈；壞 JSON 本身下一輪 /ai-work 會自癒）
      if [ "$HAVE_JQ" = 1 ]; then
    ...
    no_status)
      ...
      if [ "$nostatus_count" -ge 3 ]; then log "協定漂移過多，停止——檢查 /ai-work skill 是否還在目標 repo"; END_REASON=protocol_drift; exit 1; fi
      do_sleep "$SLEEP_BETWEEN" ;;
  esac
