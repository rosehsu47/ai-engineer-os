...
<style>
 ...
 .abtn.neutral:hover{background:#334155}
 .abtn.outline{background:transparent;color:#8b98ac;border-color:#334155}
 .abtn.outline:hover{border-color:#475569;color:#cbd5e1}
 .abtn.outline.on{border-color:#4f46e5;color:#a5b4fc;background:#4f46e51a}
 .abtn.outline-danger{background:transparent;color:#fca5a5;border-color:#7f1d1d66}
...
const ICON_PLAY='<svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"></path></svg>';
const ICON_STOP_SQUARE='<svg viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12"></rect></svg>';
// ICON_BAN：⛔ 中斷（強制 kill）用，跟 ICON_STOP_SQUARE（graceful 停止）
// 刻意用不同圖形區分——禁止／中止標準符號（圓圈+對角線），跟 STOP 那個
// 「等它自己退出」的語意不同。
const ICON_BAN='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="9"></circle><line x1="5.5" y1="5.5" x2="18.5" y2="18.5"></line></svg>';
const ICON_PIN='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21s-7-7.1-7-12a7 7 0 0 1 14 0c0 4.9-7 12-7 12z"></path><circle cx="12" cy="9" r="2.5"></circle></svg>';
// ICON_CHEVRON_SM：進階設定／顯示全部待辦的展開箭頭，跟上面 ICON_CHEVRON
// 共用風格但預設朝下（原地 rotate 180deg 表示「收合」，不用另外畫一個）。
const ICON_CHEVRON_SM='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>';
...
function supervisorBox(s){
  ...
  return '<div class="supform" data-repo="'+esc(s.path)+'">'+
    ...
    '<button class="abtn primary" data-act="supstart" data-repo="'+esc(s.path)+'">'+ICON_PLAY+'<span>啟動 supervisor</span></button>'+
    '</div>'; }
...
function persistBtn(s){
  ...
  return s.dev_server_persist_installed
    ? ' <button class="abtn outline on" data-act="devpersist-off" data-repo="'+esc(s.path)+'" title="移除開機自動啟動（不會砍掉正在跑的 process）">'+ICON_PIN+'<span>開機自動啟動</span></button>'
    : ' <button class="abtn outline" data-act="devpersist-on" data-repo="'+esc(s.path)+'" title="裝一個 launchd RunAtLoad job，開機/登入自動啟動這個 dev server">'+ICON_PIN+'<span>設開機自動啟動</span></button>';
}
  ...
  if(s.dev_command){
    if(s.dev_server_running){
      ...
      h+='<div class="row">dev server：<b style="color:#34d399">執行中</b>'+who+' '+
        ...
        '<button class="abtn neutral" data-act="devstop" data-repo="'+esc(s.path)+'">'+ICON_STOP_SQUARE+'<span>停止</span></button>'+persistBtn(s)+'</div>';
    } else h+='<div class="row">dev server：<b style="color:#8b98ac">未啟動</b> '+
      '<button class="abtn primary" data-act="devstart" data-repo="'+esc(s.path)+'">'+ICON_PLAY+'<span>啟動</span></button>'+persistBtn(s)+'</div>';
  }
...
function restoreSupState(saved){
  document.querySelectorAll('.supform[data-repo]').forEach(f=>{
    ...
    if(adv && st.adv){ adv.hidden=false; if(toggle) toggle.classList.add('open'); }
...
document.getElementById('grid').addEventListener('click', async e=>{
  ...
  if(b){
    ...
    if(b.dataset.act==='advtoggle'){
      ...
      b.classList.toggle('open', !adv.hidden);
