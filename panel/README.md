...
卡片依狀態分成 7 組顯示（依需要處理的急迫度排序，組與組之間有標題列）：
❓ 需要你回覆 → 🔴 已煞車 → 🟠 煞車生效中 → 🟢 執行中 → 🔵 已回覆待下一輪 →
...
quota 軟門檻等待中最壞要等 `quota_wait_recheck_minutes`（預設 20 分鐘）
才會生效——寫入當下 process 通常還活著。這兩個狀態分開顯示才不會誤導：
**🔴 已煞車**＝`.ai/STOP` 存在**且** supervisor.sh 的 process 已經退出，
真的停了；**🟠 煞車生效中**＝`.ai/STOP` 已經存在**但** process 還活著，
正在等下一個安全點——這個分組的卡片展開後會顯示「已送出 STOP，等下一個
安全點退出」，閒置中（沒有 `/ai-work`／`/ai-review` 在跑）的話會多一顆
FORCE STOP，不想等就可以直接按。
...
- **啟動 supervisor**（有帶 `-supervisor-script`、目前沒在跑、也沒被 STOP
  ...
  排程還是你自己在終端機起的都一定存在，永遠顯示；這個檔案本身是純
  append、supervisor.sh 沒有輪替機制，長期會無上限成長，所以連結只回
  最後 500 行，超過會在最上面提示，完整內容還在檔案裡不會被砍）、
  **`panel log`**（純文字，開新分頁看 stdout/stderr，只有 panel 自己是
  這一輪的啟動者才會有這份擷取檔，不是就不顯示——不然點下去會是 404，
  實測踩過：kotoba 由 launchd 排程啟動，這份從沒被寫過，只有 `run.log`
  能看）。同一列還有兩顆停止類
