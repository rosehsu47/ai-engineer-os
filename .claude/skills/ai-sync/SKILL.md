...
對 `templates/skills/` 下每個 skill（ai-work/ai-review/ai-task/ai-answer/ai-wrap）：
- 目標 repo 缺 → 直接複製，記入報告
- 目標 repo 有但內容與模板不同 → **直接以模板覆蓋**（CLAUDE.md 鐵律：
  修模板不修副本；repo 端的客製不該存在，若 diff 看起來像是有人刻意
  客製過，先展示 diff 問使用者再覆蓋）

**改名遷移**：已知模板改過名的 skill 目錄——`work`→`ai-work`、
`review`→`ai-review`（未來若再改名，把新的一組加進這個清單）：
- 目標 repo 的 `.claude/skills/` 下還留著舊名目錄 → 新名複製完成後，
  直接刪除舊名目錄（`rm -rf {repo}/.claude/skills/{舊名}`），記入報告；
  這是純粹的目錄搬遷、內容沒變，不用像上面客製化情況那樣先問
- 目標 repo 裡有模板從未出現過、也不在這個改名清單裡的 skill 目錄
  → 不是本輪改名的產物，可能是 repo 自己裝的或更早棄用的 skill，
  只列在報告裡提醒使用者，不主動刪
...
- 總結表：每個 repo 補了哪些 skills、刪了哪些改名後的舊 skill 目錄、
  settings 套用/待手動/略過、verify script 裝了沒
