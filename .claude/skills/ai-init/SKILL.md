...
2. **複製模板**
   ...
   - 複製 `work-record-tool/templates/skills/work/SKILL.md` →
     `{path}/.claude/skills/work/SKILL.md`，以及
     `templates/skills/review/SKILL.md` → `{path}/.claude/skills/review/SKILL.md`
     （**skills 必須住在目標 repo**——`claude -p "/work"` 在目標 repo 執行時
     只看得到那裡的 skills；deny 規則同時保證 agent 不能改寫自己的迴圈）
