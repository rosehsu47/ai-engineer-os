...
5. **驗證與收尾**
   ...
   - **安裝驗證腳本**：複製 `templates/scripts/ai-verify.sh` 到
     `{repo}/scripts/ai-verify.sh`（`scripts/` 不存在就建立）。訪談時
     問使用者有沒有想放的煙霧測試（例：health check URL）；有就直接
     填進去，沒有就保留骨架（exit 0，不擋 DoD）。跟使用者說明：
   ...
   - **註冊進 panel 清單**：把目標 repo 的絕對路徑 append 進
     ...
     panel 的清單是熱重載的，正在跑的 panel 5 秒內就會長出新卡片
