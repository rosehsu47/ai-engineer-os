---
name: review
description: AI Engineer OS 的獨立審查輪：用全新 session 審查上一個完成的任務（fresh context，非實作者自評）。由 supervisor 在 DONE_TASK 後呼叫，或人類手動 `claude -p "/review"`。
---

# /review — 獨立審查（多 agent 的第二雙眼睛）
...
4. 判定：
   ...
   - 否則 → **FAIL**：在 backlog.yaml 開一個修正任務
     ...
     `depends_on: []`），follow-up 任務會被下一輪 /work 接走
5. 在受審 receipt 尾端附加：

   ```markdown
   ## 獨立審查（/review，fresh session）
   - 判定：PASS|FAIL
   - 獨立評分：NN（實作自評：NN）
