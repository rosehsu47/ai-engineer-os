...
互動對話（人類貼截圖、即時反饋、Claude 直接改）不走 `/ai-work` 迴圈，
...
1. **盤點**：`git status --porcelain` 列出未 commit 的變更。
   ...
   - **同時跑 `git stash list`**：`git status` 乾淨不代表沒東西可收——
     supervisor 發現工作區髒時會自動 stash 非它自己任務的變更再繼續
     跑（訊息含「非本 agent 的變更」），這筆 stash 可能就是使用者以為
     已經 ship 的成果。有 stash 條目一律在盤點結果裡列出來提醒使用者，
     不要因為 `git status` 是空的就直接回報「無帳可收」
...
5. **寫帳**（三份，缺一不可）：
   ...
   - receipt + done.yaml 併入同一個 commit 或緊接著
     `chore(ai): records for T-NNN`——跟 /ai-work 的雙 commit 慣例一致
