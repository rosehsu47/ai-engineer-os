...
- 需要**禁止的操作** → 任務退回 backlog、`status: blocked` + 原因、
  寫 receipt（status: blocked）、印 `AIOS_STATUS: CONTRACT_HALT ...`，結束
- 跨**人類批准界線**（缺 secret、要裝依賴、要 migration、>10 檔、語意不明）
...
- 失敗 → 修正後重跑，**最多 2 輪修正**。仍失敗：
  ...
  任務退回 backlog（attempts 已累計；達 max_attempts 改 `status: blocked`）→
