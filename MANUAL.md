...
│  .claude/skills/{ai-work,ai-review}（隨倉出貨）   │  │
└───────────────────────────────────────────────────┘  │
        ▲                 ▲                      │
        │ 每輪全新 session  │ 審查輪（可選）        │
   claude -p "/ai-work"  claude -p "/ai-review"  │
        ▲                 ▲                      │
...
```
 cd ai-engineer-os ── claude ── /ai-init /path/to/repo
                     ...
                     └────────────────┬───────────────┘
                        ...
                        複製 ai-work/ai-review skills → {repo}/.claude/skills/
                                      ▼
                        訪談 6 題（使命/測試指令/建置指令/
...
```
     啟動 ──▶ lock？STOP？PAUSED？quota？──▶ 跑一輪 /ai-work（watchdog 30m）
              （quota 軟門檻：5h 用量 ≥60% 就不開新任務，每 20 分查一次
                /usage 等降回來自動續跑——任務不會中途斷頭，5h 就算衝到
                100% 也是等，不會停；硬門檻 80% 只看 7d，達標才寫 STOP
                保個人額度（7d 要等數天不划算）。皆可在 schedule.yml 調；
                想有意衝額度就加 `--ignore-quota`，這次不查也不寫 STOP，
                連上次 quota 寫的舊 STOP 都會一併清掉再開跑）
                                          │
 ...
 PARTIAL   EMPTY      （印問題        （睡到 reset  （指數退避      （失敗+1，
   ...
   │      正常收工                     8 輪上限）                     │
   ▼                                                                 │
 [--review] 開 fresh session /ai-review ── FAIL → 修正任務自動排入 ◀────┘
   │                                            backlog
   ▼
 睡 20s → 下一輪（直到：佇列空 / 達輪數上限 / 連續失敗 3 次 /
                  累計成本 > US$20 / quota 硬門檻 / 你 touch .ai/STOP）
```
